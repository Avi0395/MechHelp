...
exports.getRequestsByMechanic = async (req, res) => {
    ...
    try {
        console.log("Mechanic ID in backend:", mechanicId);  // Log mechanicId
        const requests = await Request.find({ mechanicId: mechanicId }).populate("userId");
        console.log("Fetched Requests:", requests);  // Log fetched requests
        res.status(200).json(requests);
    } catch (error) {
